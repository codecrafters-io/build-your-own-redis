...
`HMGET` returns the values of the specified fields, in the same order they were requested. Missing fields are returned as `nil`. If the key does not exist, every position in the reply is `nil`.

```bash
> HSET myhash field1 "Hello" field2 "World"
(integer) 2
> HMGET myhash field1 nofield field2
1) "Hello"
2) (nil)
3) "World"
...
$ redis-cli HMGET myhash field1 nofield field2
$ redis-cli HMGET nokey f1 f2
```

The tester will verify that:

- The reply to `HMGET myhash field1 nofield field2` is a RESP array of length `3`, in this exact order: `Hello`, nil bulk string, `World`.
- The reply to `HMGET nokey f1 f2` is a RESP array of length `2`, with both elements as nil bulk strings (`$-1\r\n`).
