...
```bash
> HSETNX hash_key field1 Hello
(integer) 1

# field1 already exists; the call has no effect
> HSETNX hash_key field1 World
(integer) 0
...
```bash
$ redis-cli HSETNX hash_key field1 Hello
$ redis-cli HSETNX hash_key field1 World
...
- The second `HSETNX` returns `:0\r\n`, and `HGET hash_key field1` still returns the bulk string `Hello` (the value was not overwritten).
