...
If `--appendonly yes` flag has been specified, and the `appenddirname` does not exist inside the `dir` directory, Redis creates the directory. Inside the directory, Redis creates an empty file named `<appendfilename>.1.incr.aof` inside the `appenddirname` directory.
